(function($){
  function sid(){
    try{
      var s = localStorage.getItem('dss_sid');
      if(!s){ s = 'sid_'+Math.random().toString(36).slice(2)+Date.now(); localStorage.setItem('dss_sid', s); }
      return s;
  ...
  function showGreeting(){
    var g = (window.DSS_CONFIG && DSS_CONFIG.greeting) ? DSS_CONFIG.greeting : 'سلام! از بین قیمت/دسته/کاربرد یکی را انتخاب کن.';
  ...
  function renderCards(cards){
    ...
    cards.forEach(function(c){
      ...
      if(c.url){ card.append($('<a class="dss-card-link" target="_blank">مشاهده محصول</a>').attr('href', c.url)); }
      wrap.append(card);
    });
