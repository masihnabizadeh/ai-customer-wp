    ...
    public static function handle($sid, $message){
        ...
        if ($ctx['phase']==='start'){
            ...
            if (self::contains_any($m, ['دسته','گروه','طبقه','نوع محصول','محصول'])){
                ...
                self::save($sid,$ctx);
                return ['ok'=>true,'text'=>'چه دسته‌ای مد نظرته؟','quick'=>['موبایل','لپ‌تاپ','هدفون','مودم','ساعت هوشمند']];
            }
            if (self::contains_any($m, ['استفاده','کاربرد','برای','گیم','بازی','دانشگاه','اداری','روزمره','عکاسی'])){
                $ctx['intent'] = 'usage'; $ctx['phase'] = 'ask_usage';
                self::save($sid,$ctx);
                return ['ok'=>true,'text'=>'برای چه کاری؟','quick'=>['بازی','دانشگاه','کار اداری','تولید محتوا','فیلم و موسیقی']];
            }
            self::save($sid,$ctx);
            $o = isset($opts['greeting'])?$opts['greeting']:'سلام! از بین قیمت/دسته/کاربرد یکی رو انتخاب کن.';
            return ['ok'=>true,'text'=>$o,'quick'=>['قیمت','دسته‌بندی','نوع استفاده']];
        }
        ...
        if ($ctx['phase']==='ask_budget'){
            ...
            if ($budget){
                ...
                $resp['quick'] = ['ارزان‌تر','گران‌تر','محصولات مشابه بیشتر'];
                return $resp;
            } else {
